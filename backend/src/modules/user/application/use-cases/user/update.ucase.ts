...
export default class UpdateUserUCase extends UserModel {
    ...
    public async execute({ data, id }: { data: IUpdateUserDto, id: string }) {
        const { email, firstName, lastName, phone, languaje, status, passwordHash, permissionId, role } = data;
        ...
        if (permissionId) (body as any).permissions = { connect: { id: permissionId } };
