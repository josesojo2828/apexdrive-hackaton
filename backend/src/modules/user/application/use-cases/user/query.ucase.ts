...
export default class QueryUserUCase extends UserModel {
    ...
    public async pagination({ q }: { q: QueryOptions<User, IUserQueryFilter> }) {
        const { search, skip, take, orderBy } = q as any;
        
        // REINFORCED EXTRACTION: Lookup in 'q' root AND 'q.filters'
        const qRaw = q as any;
        const filters: IUserQueryFilter = {
            status: qRaw.status || qRaw.filters?.status,
            role: qRaw.role || qRaw.filters?.role,
            email: qRaw.email || qRaw.filters?.email,
        };
