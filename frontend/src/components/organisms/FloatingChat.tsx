...
const RobotAvatar = ({ size = 28, className = "" }: { size?: number; className?: string }) => (
    ...
    >
        ...
        {/* Orb pecho */}
        <circle cx="50" cy="84" r="6" fill="var(--brand-sky)" />
        ...
        <path d="M27 47 Q33 41 39 47" stroke="var(--brand-sky)" strokeWidth="3.5" strokeLinecap="round" fill="none" />
        {/* Ojo derecho — cerrado/feliz */}
        <path d="M61 47 Q67 41 73 47" stroke="var(--brand-sky)" strokeWidth="3.5" strokeLinecap="round" fill="none" />
...
const AI_GREETING: Message = {
    id: 0,
    role: "ai",
    text: "¡Hola! Soy el asistente virtual de **Speed Delivery** \n\nPuedo ayudarte a encontrar negocios, servicios y productos de la comunidad. ¿En qué te puedo ayudar hoy?",
    timestamp: new Date(),
};
...
const MessageBubble = ({ msg }: { msg: Message }) => {
    ...
    return (
        <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
            {/* Avatar */}
            {!isUser && (
                <div className="w-8 h-8 flex-shrink-0 rounded-full bg-gradient-to-br from-[var(--brand-blue)] to-[var(--brand-sky)] flex items-center justify-center mt-1 shadow-lg shadow-blue-500/20">
                    <RobotAvatar size={35} />
                </div>
            )}
            ...
            <div className={`max-w-[80%] space-y-1 ${isUser ? "items-end" : "items-start"} flex flex-col`}>
                <div className={`px-4 py-3 rounded-[1.25rem] text-sm leading-relaxed font-sans whitespace-pre-line border backdrop-blur-md transition-all
                    ${isUser
                        ? "bg-gradient-to-br from-[var(--brand-blue)] to-[var(--brand-sky)] text-white border-[var(--brand-blue)]/50 rounded-tr-sm shadow-[0_4px_12px_rgba(0,242,254,0.2)]"
                        : "bg-white/70 dark:bg-brand-blue/80/70 text-brand-blue/80 dark:text-slate-200 border-white dark:border-slate-700 rounded-tl-sm"
                    }`}
                >
                    {parts.map((part, i) =>
                        i % 2 === 1
                ...
                <span className="text-[10px] text-brand-white/60 dark:text-slate-500 font-bold px-1 uppercase tracking-tighter">{formatTime(msg.timestamp)}</span>
            </div>
        </div>
    );
};
        ...
        <>
            ...
            >
                <GlassCard className="!p-0 border-none shadow-2xl overflow-hidden flex flex-col h-[520px] !rounded-[2.5rem]">
                    {/* Header */}
                    <div className="flex items-center gap-3 px-6 py-5 bg-gradient-to-r from-[var(--brand-blue)] to-[var(--brand-sky)] text-white relative">
                        ...
                        <div className="flex-1 min-w-0 relative z-10">
                            <Typography variant="H4" className="text-sm leading-none text-white font-black">Asistente Speed Delivery</Typography>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-hide">
                        ...
                        {isTyping && (
                            <div className="flex gap-3 items-center animate-pulse">
                                <div className="w-8 h-8 flex-shrink-0 rounded-full bg-blue-500/10 flex items-center justify-center border border-blue-500/20">
                                    <RobotAvatar size={18} />
                                </div>
                                <div className="bg-white/50 dark:bg-brand-blue/80/50 rounded-2xl rounded-tl-sm px-4 py-3 flex gap-1 items-center border border-white dark:border-slate-700 backdrop-blur-sm">
                    ...
                    {/* Input Area */}
                    <div className="p-5 border-t border-white/40 dark:border-slate-700/50 bg-white/30 dark:bg-brand-blue/40 backdrop-blur-xl">
                        <form
                        ...
                        >
                            <div className="flex-1">
                                <Input
                                    ...
                                    disabled={isTyping}
                                    className="!rounded-2xl !bg-white/60 dark:!bg-brand-blue/80/60"
                                />
                            </div>
                            <Button
                                type="submit"
                        ...
                        <Typography variant="P" className="text-[10px] text-brand-white/60 dark:text-slate-500 text-center mt-3 font-bold uppercase tracking-widest">
                            Presiona Enter para enviar · Speed AI
                        </Typography>
                    </div>
                </GlassCard>
            </div>
            ...
            >
                <GlassCard className={`!p-0 !rounded-full shadow-2xl transition-all duration-500 border-[var(--brand-blue)]/30 hover:border-[var(--brand-blue)]/60 ${isOpen ? 'rotate-90 scale-110' : 'hover:scale-110'}`}>
                    <div className="w-20 h-20 flex items-center justify-center relative overflow-hidden">
                        {isOpen ? (
                            <X className="w-8 h-8 text-[var(--brand-sky)] animate-fade-in" />
                        ) : (
                            <div className="relative">
                                <RobotAvatar size={80} className="drop-shadow-xl" />
                                {/* Notification Dot */}
                                <div className="absolute top-1 right-1 w-4 h-4 bg-emerald-400 border-4 border-white dark:border-brand-blue/80 rounded-full shadow-lg"></div>
                            </div>
                        )}
                        {/* Inner Gradient Shine */}
                        <div className="absolute inset-0 bg-gradient-to-tr from-[var(--brand-blue)]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    </div>
                </GlassCard>
            </button>
        </>
