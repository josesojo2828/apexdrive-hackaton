...
const CUSTOM_ICON_MAP: Record<string, string> = {
    ...
    'bike': '/icons/svg/cat-transport.svg',
    'common-address': '/icons/svg/srv-location-pin.svg',
    'home': '/icons/svg/ui-home.svg',
};
...
export const DynamicIcon = ({ name, className, size }: DynamicIconProps) => {
    if (!name) return null;
    ...
    if (!IconComponent) {
        return <LucideIcons.HelpCircle className={className} size={iconSize} style={style} />;
    }

    return <IconComponent className={className} size={iconSize} style={style} />;
};
