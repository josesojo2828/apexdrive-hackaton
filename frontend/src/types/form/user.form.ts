...
export const UserCreateForm: FormStructure = {
    ...
    fields: [
        ...
        { name: 'permissionId', label: 'user.permission', type: 'autocomplete', gridCols: 2, remote: { slug: 'PERMISSION_USER' }, validation: { required: true } },
    ]
};
    ...
    fields: [
        ...
        { name: 'lastName', label: 'user.lastName', type: 'text', gridCols: 2 },
        {
            name: 'status', label: 'user.status', type: 'select', gridCols: 2, options: [
                { label: 'user.status_active', value: 'ACTIVE' },
                { label: 'user.status_inactive', value: 'INACTIVE' }
            ]
        },
        {
            name: 'kycLevel', label: 'user.kyc', type: 'select', gridCols: 2, options: [
                { label: 'user.kyc_none', value: 0 },
                { label: 'user.kyc_verified', value: 1 }
            ]
        },
        { name: 'permissionId', label: 'user.permission', type: 'autocomplete', gridCols: 2, remote: { slug: 'PERMISSION_USER' } },
...
export const AddressCreateForm: FormStructure = {
    slug: 'address',
    fields: [
        { name: 'street', label: 'address.street', type: 'textarea', gridCols: 2, validation: { required: true }, placeholder: 'address.street_placeholder' },
        { name: 'city', label: 'address.city', type: 'text', gridCols: 2, placeholder: 'Ciudad...' },
