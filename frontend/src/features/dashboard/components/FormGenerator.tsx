        ...
        <Controller
            ...
            render={({ field: { onChange, value }, fieldState: { error } }) => (
                <div className="relative">
                    ...
                    >
                        <option value="">{loading ? "Cargando..." : "Seleccionar opción"}</option>
            ...
            {hasTranslatableFields && (
                <div className="flex gap-2 p-1 bg-base-200/50 rounded-2xl w-fit border border-base-300">
                    {['es', 'en', 'ja'].map(lang => (
                        ...
                        >
                            {lang === 'es' ? '🇪🇸 Español' : lang === 'en' ? '🇺🇸 Inglés' : '🇯🇵 日本語 (Japones)'}
                        </button>
                    ))}
                </div>
            )}
                    ...
                    return (
                        ...
                        >
                            {!['switch', 'checkbox_single'].includes(field.type) && (
                                <label className="block text-[10px] font-black opacity-50 uppercase tracking-widest mb-2 ml-1 italic leading-none">
                                    ...
                                    case 'number':
                                        return (
                                            <input
                                                ...
                                                className={cn("input w-full", errors[field.name] && "input-error")}
                                        ...
                                        return (
                                            <Controller
                                                ...
                                                render={({ field: { onChange, value } }) => (
                                                    <label className="flex items-center justify-between p-4 bg-base-200 rounded-2xl cursor-pointer border border-base-300">
                                                        <span className="text-[10px] font-black opacity-60 uppercase tracking-widest italic">{t(field.label)}</span>
                                                        <input
                                                            type="checkbox"
                                        ...
                                        return (
                                            <div className="flex flex-wrap gap-2">
                                                {(field.options || []).map((opt) => (
                                                    <label key={String(opt.value)} className="flex-1 min-w-[100px] cursor-pointer">
                                                        <input type="radio" value={String(opt.value)} {...register(field.name)} className="peer hidden" />
                                                        <div className="p-3 text-center border border-base-300 rounded-xl peer-checked:border-primary peer-checked:bg-primary peer-checked:text-primary-content transition-all">
                                    ...
                                    case 'textarea':
                                        return (
                                            <textarea
                                                ...
                                                className={cn("textarea w-full min-h-[100px]", currentError && "textarea-error")}
                                    ...
                                    case 'select':
                                        return (
                                            <div className="relative">
                                                <select
                                                    {...register(fieldName, { required: field.validation?.required && (!field.translatable || activeLang === 'es') })}
                                                    className={cn("select w-full", currentError && "select-error")}
                                                >
                                        ...
                                        return (
                                            <Controller
                                                ...
                                                render={({ field: { value, onChange } }) => (
                                                    <MapPicker
                                                        lat={Number(formValues['latitude']) || 9.9114}
                                                        lng={Number(formValues['longitude']) || -67.3533}
                                                        onChange={(coords) => {
                                                            onChange(coords);
                                                            // We assume the form expecting latitude and longitude separately too for backend
