...
export const useCrud = (slug: string) => {
    ...
    useEffect(() => {
        setPage(1);
    }, [debouncedSearch, orderBy, extraFilters]);
    ...
    const reloadData = useCallback(async () => {
        ...
        try {
            ...
            // Enviamos los parámetros planos. NestJS los recibirá en el objeto 'q'
            const params = {
                search: debouncedSearch || undefined, // El backend ahora busca por esto
            ...
            setTotalItems(result.total || finalData.length);
            setData(finalData);
        } catch (error) {
    ...
    }, [slug, debouncedSearch, orderBy, page, itemsPerPage, extraFilters]);
    ...
    return { 
        ...
        search, setSearch, remove, reloadData,
        extraFilters, setExtraFilters,
        pagination: { page, setPage, itemsPerPage, setItemsPerPage, totalItems }
    };
};