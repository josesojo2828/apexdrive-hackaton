...
export const validateRegisterForm = (
    ...
    const fNameError = validateName(firstName);
    if (fNameError) {
        errors.push({ field: "firstName", message: fNameError });
    }

    const lNameError = validateName(lastName);
    if (lNameError) {
        errors.push({ field: "lastName", message: lNameError });
    }
    ...
    if (!phone || phone.length < 7) {
        errors.push({ field: "phone", message: "El teléfono es requerido y debe ser válido" });
    }
